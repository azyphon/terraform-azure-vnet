...
#try(var.config.subnets[subnet].shared.network_security_group, null) == null ||
...
#condition = alltrue([
#for subnet in keys(var.config.subnets) : (
#try(var.config.subnets[subnet].shared.route_table, null) == null ||
...
variable "config" {
  ...
  type = object({
    ...
    subnets = optional(map(object({
      ...
      network_security_group_shared = optional(string)
      route_table = optional(object({
        name                          = optional(string)
        bgp_route_propagation_enabled = optional(bool, true)
        tags                          = optional(map(string))
      ...
      route_table_shared = optional(string)
    })), {})
    network_security_groups = optional(map(object({
      name = optional(string)
      tags = optional(map(string))
  ...
  validation {
    condition = alltrue([
      for subnet in keys(var.config.subnets) : (
        var.config.subnets[subnet].network_security_group_shared == null ||
        try(contains(keys(var.config.network_security_groups), var.config.subnets[subnet].network_security_group_shared), false)
      )
    ])
    error_message = "One or more subnets reference a shared network_security_group that does not exist in network_security_groups."
  }
  validation {
    condition = alltrue([
      for subnet in keys(var.config.subnets) : (
        var.config.subnets[subnet].route_table_shared == null ||
        try(contains(keys(var.config.route_tables), var.config.subnets[subnet].route_table_shared), false)
      )
    ])
    error_message = "One or more subnets reference a shared route_table that does not exist in route_tables."
  }
